...
export default class Summary extends React.Component {
  ...
  async doRequestBids(appraisal, bidRequestsMade) {
    await dmxApiService.requestBids(appraisal.id, appraisal.owner);
  ...
  async fetchData() {
    ...
    if (this.props.route.newAppraisal) {
      await appraisalModel.saveAppraiser(user.id, user.dealerId);
  ...
  async requestBids() {
    const appraisal = await appraisalModel.getCurrentAppraisal();
    if (!this.validate() || !isComplete(appraisal)) {
      return;
    }
